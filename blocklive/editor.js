...
async function onTabLoad() {
    ...
    addButtonInjectors()
    //? { 
    // todo: set BL id before this file runs
...
store.subscribe(function() {
    ...
    // HANDLE TITLE CHANGE
    let title = store.getState().scratchGui.projectTitle
    if(title != lastTitle) {
        lastTitle = title
...
let blActivateClick = async ()=>{
    ...
    chrome.runtime.sendMessage(exId,{json,meta:'create',scratchId,title:store.getState().scratchGui.projectTitle},async (response)=>{
        blId = response.id 
